...
readonly BACKUP_LOCAL_DIR="/opt/mnaprojectname/backups/metabase"
...
function backup_metabase(){
  ...
  stop_container
  tar -zcvf "/opt/mnaprojectname/backups/metabase/metabase-$(date +'%Y-%m-%d_%H%M%S').tar.gz" \
    -C /opt/mnaprojectname/data/metabase .
  restart_container
